...
DeNoise <- function(spec,
  ...
  if (detectIntensityAbs) {
    thrIntensityAbs <- max(.detect_intensity_abs(spec[, 'intensity'], thrDetectCounts), thrIntensityAbs)
  }
...
.detect_intensity_abs <- function(intensity, thrCounts = 5) {
  intcounts <- table(intensity)
  res <- 0
  if (any(isabove <- intcounts >= thrCounts)) {
    res <- max(as.numeric(names(intcounts)[isabove])) + 1
  }
  if ((len_int <- length(intensity)) < thrCounts) {
    if ((len_int > 1) && (length(int_unique <- unique(intensity)) == 1)) {
      res <- int_unique + 1
    }
  }
